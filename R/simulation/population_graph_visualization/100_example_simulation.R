...
save(models_grid, file = "data_simulation/example_sim.RData")

#### run simulation ####

# prepare populations
population_settings <- popgenerator::init_population_settings(models_grid)
populations <- popgenerator::generate_all_populations(population_settings)

# prepare relations
relations_settings <- popgenerator::init_relations_settings(models_grid, populations)
rel <- popgenerator::generate_all_relations(relations_settings)[[1]]

pop <- populations[[1]]

save(pop, file = "data_simulation/example_sim_pop.RData")
save(rel, file = "data_simulation/example_sim_rel.RData")
